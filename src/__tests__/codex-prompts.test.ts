...
describe('buildCodexGenerationPrompt', () => {
  ...
  it('keeps the generation rules, company-name exactness, and JSON-only output contract', () => {
    ...
    expect(prompt).toContain('Return ONLY valid JSON matching the output format above.')
    expect(prompt).toContain('Do not wrap the JSON in markdown.')
    expect(prompt).toContain('Verb repetition is a hard constraint.')
    expect(prompt).toContain('No verb may appear more than twice total.')
